...
export class ManageProductComponent implements OnInit {
    ...
    getApiPartners(){
        this.apiService.get('api_partner/api_partner_list').subscribe(res => {
            this.api_partners = res.data;
        });
    }

    onFilter() {
        if (this.filterData.product_type == '') {
            this.toastr.warning('Select Product Type.');
        } else if (this.filterData.branding_category == '') {
            this.toastr.warning('Select Branding Category.')
        } else {
            this.spinner.show();
        ...
        this.messageService.confirm(`Are you sure to ${active ? '' : 'In'}active Product`, `${active ? 'A' : 'Ina'}ctive Product`).then(
            value => {
                if (value.isConfirmed) {
                    ...
                    if (active) {
                        this.apiService.post('product/change-product-status', {"product_id":product.id,"product_status":true}).subscribe({
                            next: (res) => {
                        ...
                        this.apiService.post('product/change-product-status', {"product_id":product.id,"product_status":false}).subscribe({
                            next: (res) => {
                                this.toastr.success(res.message);
                                this.spinner.hide().then(r => {
                                    return r;
    ...
    onSubmit() {
        ...
        this.spinner.show();
        if (this.productID) {
            this.apiService.post(`product/${this.productID}/edit`, this.productForm.value, 'put').subscribe({
                next: (res) => {
                    this.toastr.success(res.message);
                    this.spinner.hide();
                    this.offCanvas.dismiss();
