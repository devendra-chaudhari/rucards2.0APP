...
export class MasterComponent implements OnInit {
    ...
    getMax() {
        return Math.min(this.page * this.pageSize, this.miscs.length);
    }
        ...
        this.messageService.confirm(`Are you sure to ${active ? '' : 'In'}active master`, `${active ? 'A' : 'Ina'}ctive Master`).then(
            value => {
                if (value.isConfirmed) {
                    this.spinner.show();
                    if (active) {
                        this.apiService.post(`misc/${misc.id}/edit`, misc, 'put').subscribe({
                            next: (res) => {
                                this.toastr.success(res.message);
                                this.spinner.hide();
                                this.getMaster();
                        ...
                        this.apiService.get(`misc/${misc.id}/delete`, 'delete').subscribe({
                            next: (res) => {
                                this.toastr.success(res.message);
                                this.spinner.hide();
                                this.getMaster();
