...
export class DepartmentComponent implements OnInit{
        ...
        this.messageService.confirm(`Are you sure to ${active ? '' : 'In'}active master`, `${active ? 'A' : 'Ina'}ctive Master`).then(
            value => {
                if (value.isConfirmed) {
                    this.spinner.show();
                    if (active) {
                        this.apiService.post(`department/active_department`, {"department_id":department.id}).subscribe({
                            next: (res) => {
                        ...
                        this.apiService.post(`department/inactive_department`, {"department_id":department.id}).subscribe({
                            next: (res) => {
                                this.toaster.success(res.message);
                                this.spinner.hide();
                                this.getAllDepartments();
    ...
    onEditDepartmentSubmit(){
        ...
        this.apiService.post('department/update_department', this.editDepartment).subscribe({
            next: (res) => {
                this.toaster.success(res.message);
