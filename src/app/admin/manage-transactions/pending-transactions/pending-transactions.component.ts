...
interface PendingTransactionsList {
...
export class PendingTransactionsComponent implements OnInit {
  ...
  businessReport: PendingTransactionsList[] = [];
  page: number = 1;
  pageSize: number = 10;
  categorys: Detail[] = [];
  tempExcelData = [];
  ...
  PendingTransactionsForm = new FormGroup({
  ...
  ngOnInit() {
    this.breadCrumbItems = [
      ...
      { label: 'Pending Transactions', active: true },
    ];
    this.getCategory()
  }
  ...
  onFilterModeChange(): void {
    const selectedFilterMode = this.PendingTransactionsForm.get('filter_mode')?.value;
    this.select_role = selectedFilterMode === '1'; // Show input field for 'filter mode 1'
  }
  onUserRoleChange(): void {
    const selectedUserRole = this.PendingTransactionsForm.get('specific_user')?.value;
    this.select_user = true;

  }
  
  onSubmit() {
    console.log(this.PendingTransactionsForm.value)
    // this.apiService.post('manage_notifications/add', this.PendingTransactionsForm.value).subscribe({
    //   next: (res) => {
    //     console.log('Notification saved successfully:', res);
    //       this.toastr.success(res.message);
    //       this.offCanvas.dismiss();
    //       this.PendingTransactionsForm.reset();
    //     },
    //     error: (error) => {
