...
export class RevokeBalanceRequestComponent {
  ...
  transferBalance() {
    ...
    this.apiService.post("wallet/revoke-balance", payload).subscribe({
      next: (res) => {
        this.toastr.success(res.message);
        this.transferBalanceForm.reset();
  ...
  getTransferHistory(
    ...
    console.log("in getRetailerListByDistributorId", data);
    this.apiService
      .post("transaction/get_revoke_balance_transaction", data)
      .subscribe(
        (res) => {
...
protected readonly Math = Math;
  ...
  export_to_excel() {
    this.excelService.exportAsExcelFile(this.transactions, 'Revoke-fund-request-' + random() * 56413216544 + '.xlsx', 'request_date', ['receipt', 'user_id', 'receiver_id'], ['fund_transaction_id', 'transaction_type', 'sender_profile_id ', 'amount', 'current_balance', 'created_at'])
